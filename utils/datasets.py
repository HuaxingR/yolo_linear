...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(img), labels_out, self.im_files[index], shapes, nl
    ...
    def collate_fn(batch):
        im, label, path, shapes, nl = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(im, 0), torch.cat(label, 0), path, shapes, nl
